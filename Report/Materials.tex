...
Error \: Correction = t = \frac{d_{min}-1}{2}
\end{eqnarray}
...
An other way to find the syndrome matching $ r(X) $ is to make the polynomial division as in Equation \ref{syndrom formular}. SKAL DETTE BEVISES???????????????????????????????????????????
...
\end{eqnarray*}
After 7 shift 7 the syndrome match the error pattern for one error at the last bit.
...
When introducing 2 errors correcting the error pattern is extended to the one shown in list: (INDÆST LINK HER MAIKEN!).
